...
def run(img):
    ...
    copy_img = set_region_of_interest(edge_img,vertices)

    # 5) 허프 변환을 사용하여 조건을 만족하는 직선 검출
    lines = cv2.HoughLinesP(copy_img, rho = 1, theta = np.pi/180, threshold=10, lines= None,
                            maxLineGap=None, minLineLength=None)

    # 6) 찾은 직선들을 입력 이미지에 그리기
    result = np.copy(img)
    for line in lines:
        for x1, y1, x2, y2 in line:
            # (0,0,255) -> RGB로 인해 빨간색 넘어감
            # 마지막 파라키터 두께
