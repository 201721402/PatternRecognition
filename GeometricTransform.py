...
fwd_trans_M = np.array([[1, 0, -width/2],[0,1,-height/2],[0,0,1]])
rot_M = np.array([[0,1,0],
                  [-1,0,0],
                  [0,0,1]])
bwd_tran_M = np.array([[1,0,width/2],[0,1,height/2],[0,0,1]])

tmp = np.matmul(bwd_tran_M, rot_M)
my_M = np.matmul(tmp, fwd_trans_M)
my_M = my_M[:2, ]
